...
class CSVExportService {
  ...
  static List<List<dynamic>> _generateCSVData(List<BarcodeItem> items) {
    ...
    final usbCount = items.where((item) => item.scanMethod == ScanMethod.usb).length;
    final cameraCount = items.where((item) => item.scanMethod == ScanMethod.camera).length;
    final manualCount = items.where((item) => item.scanMethod == ScanMethod.manual).length;
    
    List<List<dynamic>> csvData = [];
    
    // Cabeçalho com metadados
    csvData.add(['# Relatório de Devoluções - Gerado em ${dateFormat.format(now)} às ${timeFormat.format(now)}']);
    csvData.add(['# Total Geral: ${currencyFormat.format(totalValue)} ($totalItems itens)']);
    csvData.add(['# USB: $usbCount itens | Câmera: $cameraCount itens | Manual: $manualCount item${manualCount != 1 ? 's' : ''}']);
